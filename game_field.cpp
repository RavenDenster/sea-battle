...
bool GameField::attack_cell(int x, int y, ShipManager& manager, AbilityManager& manager_ability) { 
    try{
                    ...
                    for (int j = 0; j < ship_length; ++j) {
                        if (y == ship.get_y() + j) {
                            if(ship.get_segment_state(j) == 2) {
                                cerr << "This segment has already been destroyed." << endl;
                                return 0;
                            }
        ...
        cerr << "The field has a width of " << e.get_width_state() << " and a height of " << e.get_height_state() << endl; 
        throw;
    } 
    return 0;
}
...
void GameField::place_ship(Ship& ship, int x, int y, string orientation) {
    while (true) {
        try {
            ...
            cerr << "The ship is already located at coordinates x - " << e.get_x_state() << " y - " << e.get_y_state() << endl;
            throw;
